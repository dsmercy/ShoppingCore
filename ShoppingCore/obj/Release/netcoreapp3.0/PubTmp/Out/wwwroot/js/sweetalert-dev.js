...
exports['default'] = sweetalert = swal = function () {
  ...
  // Show swal with enabled buttons always
  swal.enableButtons();
};
...
// Modal
"<div class=\"sweet-swal\">" +

// My Custom Cancel button
 "<button id=\"CustomClick\" style=\"float:right;background-color:white !important;color:black;padding: 16px 11px;margin:0;background: url(/images/close-img.png) no-repeat\"></button>" +
...
var swalTypes = ['error', 'warning', 'info', 'success', 'input', 'prompt'];
...
var setParameters = function setParameters(params) {
  ...
  if (params.type && !_isIE8.isIE8()) {
    var _ret = (function () {
      ...
      for (var i = 0; i < swalTypes.length; i++) {
        if (params.type === swalTypes[i]) {
          validType = true;
          break;
        }
      }
      ...
      if (!validType) {
        logStr('Unknown swal type: ' + params.type);
        return {
          v: false
        };
      }
